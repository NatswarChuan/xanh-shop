...
class Ibtana_CPT {
  ...
  public function __construct() {
    ...
    //   $edit = get_post_type_object( 'ibtana_templates' );
    ...
    add_action( 'admin_head', function () {
      ...
      foreach ( $capabilities as $capability ) {
        // if ( ( strpos( $capability, 'delete_' ) !== false ) && ( $current_screen->is_block_editor ) ) {
    ...
    //   if ( 'edit_ibtana_template' == $cap || 'delete_ibtana_template' == $cap || 'read_ibtana_template' == $cap ) {
  ...
  public function ive_import_saved_single_ibtana_template() {
    ...
    // exit;
  	...
  	$single_ive_builder_template = get_post( $_POST['post_id'] );
  ...
  public function set_default_save_template_limit_info() {
    $ive_default_settings = array(
      'save_template_limit' => isset( $_POST['save_template_limit'] ) ? $_POST['save_template_limit'] : 2
    );
  ...
  public function ive_ajax_save_template() {
    ...
    if ( $template_posts->publish < $template_limit_info['save_templates_limit'] ) {
      ...
      //   wp_delete_term($custom_term->term_id, 'ibtana_template_type');
      // }
      // exit;

      $_POST_post_type = $_POST['post_type'];
      ...
      wp_send_json( [
         'status'                 =>	true,
         'msg'		                =>	"Successfully saved template '$title'.",
         'save_templates_limit'   =>  $template_limit_info['save_templates_limit'],
         'saved_templates'        =>  $template_limit_info['saved_templates'],
         'is_add_on_providing_template_limit'  =>  $template_limit_info['is_add_on_providing_template_limit']
        ]
      ...
      wp_send_json( [
         'status' =>	false,
         'msg'		=>	"Can't Save More Than " . $template_limit_info['save_templates_limit'] . " Templates."
        ]
      );
      exit;
    }
  ...
  public function ive_delete_saved_single_ibtana_template() {
    ...
    $is_deleted = wp_delete_post( $_POST['post_id'], true );
