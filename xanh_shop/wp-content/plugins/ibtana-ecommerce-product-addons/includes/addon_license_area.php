...
<div class="ibtana_addon_license_container">
  ...
  <div class="ibtana_lic_body">
    <form id="iepa-license-form">
      <input type="text" id="iepa_license_key_input" placeholder="Enter License Key" required="" <?php echo $iepa_license_status ? 'disabled' : '' ?> value="<?php echo $iepa_license_key ?>">
    ...
    $( '#iepa-license-form' ).on( 'submit', function(e) {
      ...
      jQuery.post( ive_whizzie_params.ajaxurl, iepa_data, function( iepa_response ) {
        if( iepa_response.status == true ) {
          ...
          jQuery( '#iepa-license-form button' ).text( 'Activated' );
					$( '#iepa-license-form .ive-addon-license-key-buttons-wrap' ).append(
						`<button type="reset" name="iepa_change_key">Change Key</button>`
					);
					get_iepa_activation_status( function( status ) {
						if ( true === status ) {
							location.href = ive_whizzie_params.adminUrl + "admin.php?page=ibtana-visual-editor-templates&woo=true";
						}
					} );
        } else {
          alert( iepa_response.msg );
		...
		$( '#iepa-license-form' ).on( 'click', '[name="iepa_change_key"]', function() {
			...
			$( '#iepa-license-form button[type="submit"]' ).text( 'Activate' );
